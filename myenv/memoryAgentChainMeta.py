...
pinecone.init(
    ...
    If you have extracted information from a specific page_content and the filename of that metadata is valid, please append that you obtained it from that portion.
    {context}
    ...
    workflow_search = RetrievalQA.from_chain_type(llm=llm, chain_type="stuff", retriever=docsearch.as_retriever(), chain_type_kwargs=chain_type_kwargs, return_source_documents= True)
    


    response = workflow_search({"query": input.get('question')})
    documents = response["source_documents"]
    if(documents[0].dict().get('metadata').get('filename') != None):
        return response["result"] + documents[0].dict().get('metadata').get('filename')
